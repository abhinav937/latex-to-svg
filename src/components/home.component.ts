...
@Component({
  ...
  template: `
    <div class="h-[calc(100vh-4rem)] flex flex-col bg-white dark:bg-gray-900">
      <main class="flex-1 flex relative min-h-0">
        <!-- Main Editor Area -->
        <div class="flex-1 overflow-y-auto bg-white dark:bg-gray-900 relative z-0 min-h-0">
          <app-latex-editor></app-latex-editor>
        </div>

        <!-- History Sidebar (Desktop) -->
        <aside class="w-72 xl:w-80 hidden lg:block overflow-hidden h-full shadow-lg border-l border-gray-200 dark:border-gray-700 z-10 flex-shrink-0">
